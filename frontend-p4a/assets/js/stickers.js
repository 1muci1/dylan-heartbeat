(() => {
  ...
  const list = async keyword => (await request(`/api/v1/stickers?keyword=${encodeURIComponent(keyword || "")}`)).data;
  ...
  const blobUrl = async pathname => URL.createObjectURL(await request(pathname));
  window.AppMedia = Object.freeze({ list, uploadSticker, update, remove, restore, uploadImages, blobUrl, request });
})();
