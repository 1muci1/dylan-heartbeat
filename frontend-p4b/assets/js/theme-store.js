...
((root, factory) => {
  ...
  const DESIGN_IMAGE_DEFAULT = Object.freeze({ enabled:false,url:"",opacity:1,size:"cover",position:"center",repeat:"no-repeat",blendMode:"normal",offsetX:0,offsetY:0,scale:1,blur:0,layer:"behind",clip:true });
  ...
  const normalizeCustomDesign = input => {
    ...
    for (const key of Object.keys(DESIGN_REGIONS)) {
      ...
      region.image=Object.freeze({ enabled:image.enabled===true,url:safeVisualAsset(image.url),opacity:boundedNumber(image.opacity,1,0,1),size:["cover","contain","fill","original"].includes(image.size)?image.size:"cover",position:["center","top","bottom","left","right","top-left","top-right","bottom-left","bottom-right","custom"].includes(image.position)?image.position:"center",repeat:["repeat","no-repeat"].includes(image.repeat)?image.repeat:"no-repeat",blendMode:["normal","soft-light"].includes(image.blendMode)?image.blendMode:"normal",offsetX:boundedNumber(image.offsetX,0,-100,100),offsetY:boundedNumber(image.offsetY,0,-100,100),scale:boundedNumber(image.scale,1,.1,4),blur:boundedNumber(image.blur,0,0,20),layer:["background","behind","front"].includes(image.layer)?image.layer:"behind",clip:image.clip!==false });
