...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const gatewayRequest = async (path, options = {}) => { const config = window.AppConfig?.getProviderConfig?.() || {}; const url = gateway.resolveGatewayUrl(path, { baseUrl: config.baseUrl, locationRef: window.location }); const response = await fetch(url, { ...options, headers: { ...gatewayHeaders(), ...(options.headers || {}) } }); const payload = await response.json().catch(() => ({})); if (!response.ok) { const error=new Error(response.status===401?"主题素材接口认证失败，请检查当前 Gateway 配置。":payload.error?.message||`请求失败（${response.status}）`);error.status=response.status;error.code=payload.error?.code;throw error; } return payload; };
  ...
  const renderSlotEditor = () => {
    ...
    for (const [key, labelText] of Object.entries(slotLabels)) {
      ...
      const choose=document.createElement("button");choose.type="button";choose.textContent="选择素材";choose.addEventListener("click",()=>{selectedSlotKey=key;openPicker({type:"slot",key});});
  ...
  const renderDesignForm = () => {
    ...
    const current=document.querySelector("[data-design-current-asset]"),imageStatus=document.querySelector("[data-design-image-status]");if(current){current.replaceChildren();const asset=serverAssets.find(item=>item.url===image.url)||(draft.assetLibrary||[]).find(item=>item.url===image.url),src=asset&&safeAssetImage(asset.url);if(src){const img=document.createElement("img");img.alt="当前本地素材";img.addEventListener("load",()=>{if(imageStatus)imageStatus.textContent=`图片状态：✓ 已加载 · ${img.naturalWidth}×${img.naturalHeight}`;});img.addEventListener("error",()=>{if(imageStatus)imageStatus.textContent="图片状态：× 加载失败";});img.src=src;const text=document.createElement("span");text.textContent=`${kindLabel(asset.kind)} · ${asset.id.slice(0,8)}`;current.append(img,text);if(imageStatus)imageStatus.textContent=asset.width&&asset.height?`图片状态：✓ 已加载 · ${asset.width}×${asset.height}`:img.complete&&img.naturalWidth>0?`图片状态：✓ 已加载 · ${img.naturalWidth}×${img.naturalHeight}`:"图片状态：加载中…";}else{current.textContent="未选择素材";if(imageStatus)imageStatus.textContent="图片状态：等待选择素材";}}
    syncDesignValues();
  };
  ...
  const selectPreviewRegion=key=>{if(!api.DESIGN_REGIONS[key])return;selectedDesignRegion=key;selectedDesignPage=key.split(".")[0];designPreviewTab=["chat","home"].includes(selectedDesignPage)?selectedDesignPage:"generic";renderDesignNavigation();renderDesignForm();renderDesignPreview();};const designPreview=document.querySelector("[data-design-preview]");designPreview?.addEventListener("click",event=>{const regionNode=event.target.closest("[data-design-preview-region]");if(regionNode)selectPreviewRegion(regionNode.dataset.designPreviewRegion);});designPreview?.addEventListener("pointerdown",event=>{let layer=event.target.closest(".advanced-preview__image-layer");if(!layer){const selected=event.target.closest("[data-design-preview-region]");if(!selected||selected.dataset.designPreviewRegion!==selectedDesignRegion)return;layer=selected.querySelector(":scope > .advanced-preview__image-layer");}if(!layer)return;const regionNode=layer.parentElement,key=regionNode.dataset.designPreviewRegion;selectPreviewRegion(key);const image=designRegion(key).image;if(!image.enabled||!safeAssetImage(image.url))return;event.preventDefault();event.stopPropagation();beginDesignHistory();const startX=event.clientX,startY=event.clientY,originX=image.offsetX,originY=image.offsetY,dragSurface=event.target instanceof Element?event.target:layer;dragSurface.setPointerCapture?.(event.pointerId);const move=moveEvent=>{const x=Math.max(-100,Math.min(100,originX+moveEvent.clientX-startX)),y=Math.max(-100,Math.min(100,originY+moveEvent.clientY-startY));updateDesignLive(next=>{const target=ensureDesignRegion(next,key);target.image.position="custom";target.image.offsetX=Math.round(x);target.image.offsetY=Math.round(y);});const position=document.querySelector('[data-design-image-field="position"]');if(position)position.value="custom";for(const field of ["offsetX","offsetY"]){const input=document.querySelector(`[data-design-image-field="${field}"]`);if(input)input.value=field==="offsetX"?Math.round(x):Math.round(y);}syncDesignValues();};const end=()=>{dragSurface.removeEventListener("pointermove",move);dragSurface.removeEventListener("pointerup",end);dragSurface.removeEventListener("pointercancel",end);commitDesignHistory();};dragSurface.addEventListener("pointermove",move);dragSurface.addEventListener("pointerup",end);dragSurface.addEventListener("pointercancel",end);});
  ...
  const categoryLabels=Object.freeze({background:"背景",bubble:"气泡",avatar:"头像",header:"顶栏",composer:"输入栏",card:"卡片",decoration:"装饰",nav:"导航",other:"其他"});
  const sourceLabels=Object.freeze({upload:"本地上传",localized:"网络导入",history:"历史素材"});
  const assetGroup=kind=>/background/iu.test(kind)?"background":/bubble/iu.test(kind)?"bubble":/avatar/iu.test(kind)?"avatar":/input|composer/iu.test(kind)?"composer":/header/iu.test(kind)?"header":/card/iu.test(kind)?"card":/nav/iu.test(kind)?"nav":/decor/iu.test(kind)?"decoration":"other";
  ...
  const themeAsset=asset=>({...asset,url:asset.url||asset.localUrl,kind:asset.kind||({background:"backgroundImage",bubble:"bubbleDecoration",avatar:"avatarFrame",header:"headerDecoration",composer:"inputDecoration",nav:"navIcon"}[asset.category]||"decorativeAsset")});
  const rememberAsset=asset=>{const item=themeAsset(asset),next=editable(draft);next.assetLibrary=(next.assetLibrary||[]).filter(candidate=>candidate.url!==item.url);next.assetLibrary.push({id:item.id,kind:item.kind,url:item.url});draft=editable(api.normalizeTheme(next));return item;};
  const chooseLibraryAsset=raw=>{const asset=rememberAsset(raw);if(assetPickerTarget.type==="slot")update(next=>{next.visualSlots.enabledByUser=true;next.visualSlots[assetPickerTarget.key].url=asset.url;next.visualSlots[assetPickerTarget.key].enabled=true;next.migratedVisualSlotsSafe=true;});else if(assetPickerTarget.type==="region")designAction(next=>{const region=ensureDesignRegion(next,selectedDesignRegion);region.enabled=true;region.image=region.image||editable(api.DESIGN_IMAGE_DEFAULT);region.image.url=asset.url;region.image.enabled=true;});picker?.close();};
  const loadServerAssets=async()=>{const payload=await gatewayRequest("/api/theme/assets/library");serverAssets=(payload.data.items||[]).map(themeAsset);renderDesignAssets();return serverAssets;};
  const referencesFor=asset=>{let savedDraft=null;try{savedDraft=JSON.parse(localStorage.getItem(DESIGN_DRAFT_KEY)||"null");}catch{}return api.collectThemeAssetReferences(asset.url,{activeTheme:store.getActive(),currentTheme:draft,draft:savedDraft?{customDesign:savedDraft}:null,savedThemes:store.getLibrary()});};
  const showReferences=references=>{const dialog=document.querySelector("[data-asset-delete-blocked]"),list=dialog?.querySelector("[data-asset-reference-list]");if(!dialog||!list)return;dialog.querySelector("[data-asset-reference-summary]").textContent=`这张素材正在被 ${references.length} 个位置使用。`;list.replaceChildren(...references.map(item=>{const li=document.createElement("li");li.textContent=`${item.scope} · ${item.location}`;return li;}));dialog.showModal();};
  const updateAssetMetadata=async(asset,label,category)=>{await gatewayRequest(`/api/theme/assets/${encodeURIComponent(asset.id)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({label,category})});await loadServerAssets();message("素材信息已更新。");};
  const deleteAsset=async asset=>{const references=referencesFor(asset);if(references.length){showReferences(references);return;}if(!window.confirm(`删除「${asset.label}」？删除后将移入安全回收目录。`))return;await gatewayRequest(`/api/theme/assets/${encodeURIComponent(asset.id)}`,{method:"DELETE"});await loadServerAssets();message("素材已删除。");};
  const renderDesignAssets=()=>{if(!assetGrid)return;assetGrid.replaceChildren();const search=document.querySelector("[data-design-asset-search]")?.value.trim().toLowerCase()||"",unusedOnly=document.querySelector("[data-design-asset-unused]")?.checked,themeOnly=document.querySelector("[data-design-asset-theme]")?.checked,source=document.querySelector("[data-design-asset-source]")?.value||"all",sort=document.querySelector("[data-design-asset-sort]")?.value||"newest",current=assetPickerTarget.type==="slot"?draft.visualSlots?.[assetPickerTarget.key]?.url:designRegion(selectedDesignRegion).image?.url,themeUrls=new Set((draft.assetLibrary||[]).map(asset=>asset.url));let assets=serverAssets.filter(asset=>{const used=assetUsage(asset.url);return safeAssetImage(asset.url)&&(assetFilter==="all"||asset.category===assetFilter)&&(!search||`${asset.label} ${asset.category} ${asset.id}`.toLowerCase().includes(search))&&(source==="all"||asset.source===source)&&(!unusedOnly||!used.length)&&(!themeOnly||themeUrls.has(asset.url));});assets.sort((a,b)=>sort==="size"?b.bytes-a.bytes:sort==="dimensions"?(b.width||0)*(b.height||0)-(a.width||0)*(a.height||0):sort==="unused"?assetUsage(a.url).length-assetUsage(b.url).length:String(b.createdAt).localeCompare(String(a.createdAt)));const usedCount=serverAssets.filter(asset=>assetUsage(asset.url).length).length,uploads=serverAssets.filter(asset=>asset.source==="upload").length,localized=serverAssets.filter(asset=>asset.source==="localized").length,stats=document.querySelector("[data-design-asset-stats]");if(stats)stats.textContent=`我的素材 ${serverAssets.length} · 本地上传 ${uploads} · 网络导入 ${localized} · 使用中 ${usedCount}`;for(const asset of assets){const src=safeAssetImage(asset.url),used=assetUsage(asset.url),card=document.createElement("article");card.className=`design-asset-choice${asset.url===current?" is-active":""}`;card.dataset.assetId=asset.id;const thumb=document.createElement("button");thumb.type="button";thumb.className="design-asset-thumb checkerboard";thumb.setAttribute("aria-label","放大查看素材");const img=document.createElement("img");img.src=src;img.alt=asset.label;img.loading="lazy";thumb.append(img);thumb.addEventListener("click",()=>openAssetPreview({...asset,localUrl:asset.url,kind:asset.kind,selector:used.join("、")||"未使用",sourceUrl:"本地素材库"}));const label=document.createElement("strong");label.textContent=asset.label;const meta=document.createElement("span");meta.textContent=`${categoryLabels[asset.category]||"其他"} · ${asset.id.slice(0,8)}`;const dimensions=document.createElement("small");dimensions.textContent=`${asset.width||"?"}×${asset.height||"?"} · ${(asset.bytes/1024).toFixed(asset.bytes>1024*1024?0:1)} KB · ${sourceLabels[asset.source]||"历史素材"}`;const usage=document.createElement("small");usage.textContent=used.length?`使用中：${used.join("、")}`:"未使用";const choose=document.createElement("button");choose.type="button";choose.textContent="选择";choose.hidden=assetPickerTarget.type==="manager";choose.addEventListener("click",()=>chooseLibraryAsset(asset));card.append(thumb,label,meta,dimensions,usage,choose);if(assetPickerTarget.type==="manager"){const edit=document.createElement("div");edit.className="design-asset-edit";const name=document.createElement("input");name.value=asset.label;name.maxLength=80;name.setAttribute("aria-label","素材名称");const category=document.createElement("select");category.setAttribute("aria-label","素材分类");for(const [value,text] of Object.entries(categoryLabels)){const option=document.createElement("option");option.value=value;option.textContent=text;option.selected=value===asset.category;category.append(option);}const save=document.createElement("button");save.type="button";save.textContent="保存名称/分类";save.addEventListener("click",()=>updateAssetMetadata(asset,name.value,category.value).catch(error=>message(error.message)));const remove=document.createElement("button");remove.type="button";remove.textContent="删除";remove.addEventListener("click",()=>deleteAsset(asset).catch(error=>message(error.message)));edit.append(name,category,save,remove);card.append(edit);}assetGrid.append(card);}if(!assetGrid.children.length)assetGrid.textContent="没有符合条件的素材。";};
  const openPicker=async target=>{assetPickerTarget=target;document.querySelector("[data-design-picker-title]").textContent=target.type==="manager"?"我的素材":"选择素材";assetGrid.textContent="正在读取我的素材…";picker?.showModal();try{await loadServerAssets();}catch(error){assetGrid.textContent=error.message||"素材库加载失败";}};
  document.querySelector("[data-design-open-picker]")?.addEventListener("click",()=>openPicker({type:"region"}));document.querySelectorAll("[data-open-asset-manager]").forEach(button=>button.addEventListener("click",()=>openPicker({type:"manager"})));document.querySelector("[data-design-picker-close]")?.addEventListener("click",()=>picker?.close());document.querySelectorAll("[data-design-asset-filters] [data-filter]").forEach(button=>button.addEventListener("click",()=>{assetFilter=button.dataset.filter;renderDesignAssets();}));for(const selector of ["[data-design-asset-search]","[data-design-asset-sort]","[data-design-asset-source]","[data-design-asset-unused]","[data-design-asset-theme]"])document.querySelector(selector)?.addEventListener("input",renderDesignAssets);
  document.querySelector("[data-asset-reference-close]")?.addEventListener("click",()=>document.querySelector("[data-asset-delete-blocked]")?.close());
  const uploadInput=document.querySelector("[data-design-upload-input]"),uploadStatus=document.querySelector("[data-design-upload-status]");document.querySelector("[data-design-upload]")?.addEventListener("click",()=>uploadInput?.click());uploadInput?.addEventListener("change",async()=>{const file=uploadInput.files?.[0];uploadInput.value="";if(!file)return;if(file.size>2*1024*1024){uploadStatus.textContent="图片超过允许大小。";return;}if(!["image/png","image/jpeg","image/webp"].includes(file.type)){uploadStatus.textContent="不支持这种图片格式。";return;}uploadStatus.textContent="正在上传…";try{const body=new FormData();body.append("file",file);const payload=await gatewayRequest("/api/theme/assets/upload",{method:"POST",body}),asset=themeAsset(payload.data);serverAssets.unshift(asset);renderDesignAssets();document.querySelector(`[data-asset-id="${asset.id}"]`)?.scrollIntoView?.({block:"center"});if(assetPickerTarget.type!=="manager")chooseLibraryAsset(asset);uploadStatus.textContent="已添加到我的素材";message("已添加到我的素材。");}catch(error){uploadStatus.textContent=error.status===413?"图片超过允许大小。":error.status===415?"不支持这种图片格式。":error.message||"服务器保存失败。";}});
