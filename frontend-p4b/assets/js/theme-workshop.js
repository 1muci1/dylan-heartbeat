...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const renderDesignForm = () => {
    ...
    const current=document.querySelector("[data-design-current-asset]");if(current){current.replaceChildren();const asset=(draft.assetLibrary||[]).find(item=>item.url===image.url);if(asset&&safeAssetImage(asset.url)){const img=document.createElement("img");img.src=safeAssetImage(asset.url);img.alt="当前本地素材";const text=document.createElement("span");text.textContent=`${asset.kind} · ${asset.id.slice(0,8)}`;current.append(img,text);}else current.textContent="未选择素材";}
    syncDesignValues();
  };
  ...
  const picker=document.querySelector("[data-design-asset-picker]"),assetGrid=document.querySelector("[data-design-asset-grid]");let assetFilter="all";const assetGroup=kind=>/background/iu.test(kind)?"background":/bubble/iu.test(kind)?"bubble":/avatar/iu.test(kind)?"avatar":/input/iu.test(kind)?"input":/card/iu.test(kind)?"card":/(?:decor|nav|header)/iu.test(kind)?"decor":"other";const renderDesignAssets=()=>{if(!assetGrid)return;assetGrid.replaceChildren();const current=designRegion(selectedDesignRegion).image?.url;for(const asset of draft.assetLibrary||[]){const src=safeAssetImage(asset.url);if(!src||(assetFilter!=="all"&&assetGroup(asset.kind)!==assetFilter))continue;const card=document.createElement("button");card.type="button";card.className=`design-asset-choice${asset.url===current?" is-active":""}`;const img=document.createElement("img");img.src=src;img.alt="本地素材缩略图";img.loading="lazy";const label=document.createElement("span");label.textContent=`${asset.kind} · ${asset.id.slice(0,8)}`;card.append(img,label);card.addEventListener("click",()=>{designAction(next=>{const region=ensureDesignRegion(next,selectedDesignRegion);region.image=region.image||editable(api.DESIGN_IMAGE_DEFAULT);region.image.url=asset.url;region.image.enabled=true;});picker.close();});assetGrid.append(card);}if(!assetGrid.children.length)assetGrid.textContent="没有符合筛选条件的已本地化素材。";};document.querySelector("[data-design-open-picker]")?.addEventListener("click",()=>{renderDesignAssets();picker?.showModal();});document.querySelector("[data-design-picker-close]")?.addEventListener("click",()=>picker?.close());document.querySelectorAll("[data-design-asset-filters] [data-filter]").forEach(button=>button.addEventListener("click",()=>{assetFilter=button.dataset.filter;renderDesignAssets();}));
