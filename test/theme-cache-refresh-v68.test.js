...
test("v68 service worker replaces old shell caches and keeps HTML network-first", () => {
  ...
  assert.match(sw, /CACHE_NAME = "xinban-shell-v70-p4b"/u);
  assert.match(sw, /BUILD_REVISION = "v70-p4b-theme-asset-experience"/u);
  ...
  const shouldDelete = key => key.startsWith("xinban-shell-") && key !== "xinban-shell-v70-p4b";
  assert.equal(shouldDelete("xinban-shell-v67-p4b"), true);
  assert.equal(shouldDelete("xinban-shell-v70-p4b"), false);
