...
test("game outer UI reads the active theme while board and canvas remain functional", () => {
  ...
  assert.match(gameHtml, /\/assets\/css\/theme\.css\?v=v66-p4b/u);
  assert.match(gameHtml, /\/assets\/js\/theme-store\.js\?v=v66-p4b/u);
...
test("service worker publishes the v60 theme coverage revision", () => {
  const sw = read("sw.js");
  assert.match(sw, /CACHE_NAME = "xinban-shell-v66-p4b"/u);
  assert.match(sw, /BUILD_REVISION = "v66-p4b-visual-theme-editor"/u);
});
