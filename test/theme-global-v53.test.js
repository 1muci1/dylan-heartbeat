...
test("chat loads the global theme after its legacy CSS and real chat selectors use tokens", () => {
  ...
  assert.ok(html.indexOf("assets/css/chat.css?v=v73-p4b") < html.indexOf("assets/css/theme.css?v=v73-p4b"));
...
test("all principal pages share one ThemeStore active key and v53 cache", () => {
  for (const page of ["chat.html", "settings.html", "theme-workshop.html", "index.html", "dashboard.html", "memory.html", "stickers.html"]) {
    const html = read(page); assert.match(html, /assets\/js\/theme-store\.js\?v=v73-p4b/u, page); assert.match(html, /assets\/css\/theme\.css\?v=v73-p4b/u, page);
  }
  assert.match(read("sw.js"), /xinban-shell-v73-p4b/u);
