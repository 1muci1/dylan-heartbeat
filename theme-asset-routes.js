...
function registerThemeAssetRoutes(app, { localizer, store, previewService, previewStore, presetStore = null, apiKey = process.env.GATEWAY_API_KEY } = {}) {
  ...
  app.delete("/api/theme/assets/:id", { preHandler: auth }, run(async req => {
    const references=presetStore?.findAssetReferences(req.params.id)||[];
    if(references.length)throw new ThemeAssetError("素材仍被主题预设引用",409,"THEME_ASSET_REFERENCED_BY_PRESET");
    return { ok:true,data:store.delete(req.params.id) };
  }));
