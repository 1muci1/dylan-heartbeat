...
function registerThemePresetRoutes(app,{store,apiKey=process.env.GATEWAY_API_KEY}={}) {
  ...
  app.get("/api/theme/presets/:id/versions",{preHandler:auth},run(req=>({ok:true,data:{items:store.versions(req.params.id)}})));
  app.post("/api/theme/presets/:id/versions",{preHandler:auth},run(req=>({ok:true,data:store.createVersion(req.params.id,req.body||{})})));
  app.post("/api/theme/presets/:id/versions/:versionId/restore",{preHandler:auth},run(req=>({ok:true,data:store.restoreVersion(req.params.id,req.params.versionId)})));
