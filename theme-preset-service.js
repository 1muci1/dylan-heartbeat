...
class ThemePresetStore {
  ...
  public(item) { const existing=this.assets(),references=assetReferences(item.theme,item.coverAssetId),missingAssets=references.filter(ref=>!existing.has(ref.assetId)),count=value=>Number.isSafeInteger(value)&&value>=0?value:0,copy=JSON.parse(JSON.stringify(item)),versions=Array.isArray(copy.versions)?copy.versions:[];delete copy.versions;return {...copy,versionCount:versions.length,favorite:item.favorite===true,usageCount:count(item.usageCount),lastUsedAt:item.lastUsedAt||null,visibility:item.visibility==="public"?"public":"private",communityFavorite:item.communityFavorite===true,favoriteCount:count(item.favoriteCount),viewCount:count(item.viewCount),share:item.share?.enabled&&SHARE_CODE.test(item.share.code)?{enabled:true,code:item.share.code,createdAt:item.share.createdAt||null,expiresAt:item.share.expiresAt||null}:{enabled:false,code:"",createdAt:null,expiresAt:null},assetReferences:references,assetCount:references.length,missingAssets}; }
  ...
  input(value, current = null) {
    ...
    const allowed=new Set(["name","description","tags","coverAssetId","mood","style","colorProfile","favorite","visibility","theme"]),unknown=Object.keys(value).find(key=>!allowed.has(key));if(unknown)throw new ThemePresetError(`不允许字段：${unknown}`);
    ...
    if(Object.hasOwn(value,"visibility")&&!['private','public'].includes(value.visibility))throw new ThemePresetError("公开状态无效",400,"THEME_PRESET_VISIBILITY_INVALID");
    return {
      ...
      visibility:Object.hasOwn(value,"visibility")?value.visibility:(current?.visibility==="public"?"public":"private"),
      theme:Object.hasOwn(value,"theme")?sanitizeTheme(value.theme):current?.theme
    };
  }
  create(value) { const items=this.read();if(items.length>=MAX_PRESETS)throw new ThemePresetError("主题预设数量已达上限",409,"THEME_PRESET_LIMIT");const now=this.clock().toISOString(),data=this.input(value);if(!data.theme)throw new ThemePresetError("主题不能为空");const item={id:crypto.randomUUID(),...data,versions:[],visibility:data.visibility||"private",communityFavorite:false,favoriteCount:0,viewCount:0,share:{enabled:false,code:"",createdAt:null,expiresAt:null},createdAt:now,updatedAt:now,lastUsedAt:null,usageCount:0};items.push(item);this.write(items);this.record("theme_preset.created",item.id);return this.public(item); }
  update(id,value) { const items=this.read(),index=items.findIndex(item=>item.id===id);if(index<0)throw new ThemePresetError("主题预设不存在",404,"THEME_PRESET_NOT_FOUND");const before=items[index],favoriteChanged=Object.hasOwn(value||{},"favorite")&&value.favorite!==(before.favorite===true),visibilityChanged=Object.hasOwn(value||{},"visibility")&&value.visibility!==(before.visibility==="public"?"public":"private"),data=this.input(value,before);items[index]={...before,...data,updatedAt:this.clock().toISOString()};this.write(items);if(visibilityChanged)this.record(data.visibility==="public"?"theme_preset.published":"theme_preset.unpublished",id);if(favoriteChanged)this.record("theme_preset.favorite_changed",id);if(!visibilityChanged&&!favoriteChanged)this.record("theme_preset.updated",id);return this.public(items[index]); }
  ...
  versions(id) { const item=this.get(id);return (Array.isArray(item.versions)?item.versions:[]).map(version=>({...JSON.parse(JSON.stringify(version)),snapshot:sanitizeTheme(version.snapshot)})).reverse(); }
  createVersion(id,value={}) { if(!value||typeof value!=="object"||Array.isArray(value)||Object.keys(value).some(key=>key!=="name"))throw new ThemePresetError("版本设置无效",400,"THEME_PRESET_VERSION_INVALID");const items=this.read(),index=items.findIndex(item=>item.id===id);if(index<0)throw new ThemePresetError("主题预设不存在",404,"THEME_PRESET_NOT_FOUND");const createdAt=this.clock().toISOString(),versionId=crypto.randomUUID(),version={id:versionId,presetId:id,name:Object.hasOwn(value,"name")?safeText(value.name,"版本名称",{max:80}):`版本 ${createdAt}`,createdAt,snapshot:sanitizeTheme(items[index].theme)},versions=[...(Array.isArray(items[index].versions)?items[index].versions:[]),version].slice(-MAX_VERSIONS);items[index]={...items[index],versions};this.write(items);this.record("theme_preset.version_created",id,{versionId});return JSON.parse(JSON.stringify(version)); }
  restoreVersion(id,versionId) { const items=this.read(),index=items.findIndex(item=>item.id===id);if(index<0)throw new ThemePresetError("主题预设不存在",404,"THEME_PRESET_NOT_FOUND");const version=(Array.isArray(items[index].versions)?items[index].versions:[]).find(candidate=>candidate.id===versionId);if(!version)throw new ThemePresetError("主题版本不存在",404,"THEME_PRESET_VERSION_NOT_FOUND");const theme=sanitizeTheme(version.snapshot);this.record("theme_preset.version_restored",id,{versionId});return{presetId:id,versionId,draft:{...JSON.parse(JSON.stringify(theme)),id:`preset_draft_${id}`,name:items[index].name,source:"custom",customCss:""},preset:this.public(items[index])}; }
  apply(id) { const result=this.previewApply(id),version=this.createVersion(id,{name:"应用前自动版本"}),items=this.read(),index=items.findIndex(item=>item.id===id),now=this.clock().toISOString(),usageCount=(Number.isSafeInteger(items[index].usageCount)&&items[index].usageCount>=0?items[index].usageCount:0)+1;items[index]={...items[index],lastUsedAt:now,usageCount,updatedAt:items[index].updatedAt};this.write(items);this.record("theme_preset.applied",id);return{...result,versionId:version.id,usageCount,lastUsedAt:now}; }
  ...
  communityView(item) { const value=this.public(item);if(value.visibility!=="public")throw new ThemePresetError("公开主题不存在",404,"THEME_COMMUNITY_NOT_FOUND");return{id:value.id,name:value.name,description:value.description,tags:[...value.tags],mood:value.mood,style:value.style,colorProfile:value.colorProfile,coverAssetId:value.coverAssetId,theme:sanitizeTheme(value.theme),assetReferences:value.assetReferences,missingAssets:value.missingAssets,communityFavorite:value.communityFavorite,favoriteCount:value.favoriteCount,viewCount:value.viewCount,usageCount:value.usageCount,createdAt:value.createdAt,updatedAt:value.updatedAt}; }
  community({query="",tag="",sort="latest",cursor="",limit=24}={}) { const search=safeText(query,"搜索",{max:120}).toLowerCase(),selectedTag=safeText(tag,"标签",{max:24}),pageSize=Math.max(1,Math.min(MAX_LIBRARY_PAGE,Number.parseInt(limit,10)||24)),offset=Math.max(0,Number.parseInt(cursor,10)||0),time=value=>Date.parse(value||"")||0,tie=(a,b)=>String(a.id).localeCompare(String(b.id));let items=this.read().filter(item=>(item.visibility==="public")).map(item=>this.communityView(item)).filter(item=>{const haystack=[item.name,item.description,...item.tags,item.mood,item.style,item.colorProfile].join(" ").toLowerCase();return(!search||haystack.includes(search))&&(!selectedTag||item.tags.includes(selectedTag));});items.sort((a,b)=>sort==="popular"?b.favoriteCount-a.favoriteCount||b.viewCount-a.viewCount||b.usageCount-a.usageCount||time(b.updatedAt)-time(a.updatedAt)||tie(a,b):sort==="mostUsed"?b.usageCount-a.usageCount||b.viewCount-a.viewCount||time(b.updatedAt)-time(a.updatedAt)||tie(a,b):time(b.updatedAt)-time(a.updatedAt)||tie(a,b));const total=items.length,next=offset+pageSize;return{items:items.slice(offset,next),total,nextCursor:next<total?String(next):null}; }
  communityDetail(id) { const items=this.read(),index=items.findIndex(item=>item.id===id&&item.visibility==="public");if(index<0)throw new ThemePresetError("公开主题不存在",404,"THEME_COMMUNITY_NOT_FOUND");items[index]={...items[index],viewCount:(Number.isSafeInteger(items[index].viewCount)&&items[index].viewCount>=0?items[index].viewCount:0)+1};this.write(items);return this.communityView(items[index]); }
  copyCommunity(id) { const source=this.communityView(this.get(id)),existing=this.assets(),copy=this.create({name:`${source.name} (副本)`,description:source.description,tags:source.tags,coverAssetId:source.coverAssetId&&existing.has(source.coverAssetId)?source.coverAssetId:"",mood:source.mood,style:source.style,colorProfile:source.colorProfile,visibility:"private",theme:source.theme}),items=this.read(),index=items.findIndex(item=>item.id===copy.id);items[index]={...items[index],visibility:"private",communityFavorite:false,favoriteCount:0,viewCount:0,usageCount:0,lastUsedAt:null,share:null};this.write(items);return this.public(items[index]); }
  favoriteCommunity(id,enabled) { const items=this.read(),index=items.findIndex(item=>item.id===id&&item.visibility==="public");if(index<0)throw new ThemePresetError("公开主题不存在",404,"THEME_COMMUNITY_NOT_FOUND");const current=items[index].communityFavorite===true,next=enabled===true;if(current===next)return this.communityView(items[index]);const count=Number.isSafeInteger(items[index].favoriteCount)&&items[index].favoriteCount>=0?items[index].favoriteCount:0;items[index]={...items[index],communityFavorite:next,favoriteCount:next?count+1:Math.max(0,count-1)};this.write(items);if(next)this.record("theme_preset.community_favorited",id);return this.communityView(items[index]); }
